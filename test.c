...
struct BTreeNode* CreateHuffman(ElemType a[], int n)
{
    ...
    for (i = 1; i < n; i++)//进行 n-1 次循环建立哈夫曼树
    {
        ...
        {
            if (b[j] != NULL && min == -1)   //注意条件的判断　　不要只顾着一个条件　
            {
                min = j;
                continue;
            }
        ...
        for (j = ci_min; j < n; j++)//从当前森林中求出最小权值树和次最小
        {
            if (b[j] != NULL)
            {
        ...
        q->data = b[min]->data + b[ci_min]->data;
        q->left = b[min];
        q->right = b[ci_min];

        b[min] = q;//将指向新树的指针赋给b指针数组中k1位置
        b[ci_min] = NULL;//k2位置为空
    }
    free(b); //删除动态建立的数组b
